...
| 2026-08-12 | 第 73 轮（本提交） | 完成 Canonical P2-10：将 46 套私有真题构建为可追溯、内容寻址、按词分片加载的学习卡真题原句，语料与生成物均不进入 Git。 |
| 2026-08-14 | 第 79 轮（`137092a`） | 全量生成三套私有学习数据：25340 条义项考频、25340 条原创释义例句、6549 条诚实词根拆解与助记；三条独立离线流水线（断点续跑、多轮盲判/双审/事实核查）+ 只读 loader 与三级优先级接入；全部 content-addressed 私有分片，零暂存、未 push。 |
...
## Canonical P2-10 考研真题例句库（第 73 轮）

本轮：2026-08-12。

- 只读核验 1998–2026 共 46 套私有真题，严格验证 manifest、标题、URL、年份/卷型、正文标记、正文长度与逐文件哈希；本轮不访问来源站、不运行抓取器，原始语料与派生句库均忽略且不提交。
- 新增确定性构建器，只从全文正文中提取可连续追溯的完整英文句，排除答案、Directions、选项、填空、作文、解析和结构残片；按红宝书真实 wordId 做保守精确表面匹配，每词最多 3 条。
- 生成 25 个内容寻址 shard 与严格 manifest：46 套试卷、4,845 个候选、4,444 个合法来源句、8,894 条例句，覆盖 4,095 个红宝书词；最大 shard 518,488 字节。覆盖只表示静态语料命中，不表示学习效果或掌握度。
- 运行时仅按当前词前缀加载单个 shard，校验结构、SHA-256、字节数和内容寻址文件名，并复用 Promise 缓存；缺文件、404、hash/结构异常静默降级为空，不请求外网、不新增 API 或持久化。
...
## 全站 UI/UX 深度优化：视觉收束（第 75 轮）

本轮：2026-08-12。分支 `codex/follow-up-hardening`；起始 HEAD `a1ff948`（工作区含上一轮遗留未提交改动，按用户指示先提交为 `09ab5ee`）。

- 定位：不重新设计、不加新功能、不改学习算法，把已有 UI 收束为统一成熟产品。`globals.css` `:root` 从 11 个 token 扩展为完整设计系统（sidebar/topbar/content 尺寸、文字层级、`--accent*`/`--warning*`、绿色调边框、`--radius-sm~2xl`、动效），全部从既有配色提取，未换品牌色；Sidebar Active 从「白卡+阴影」弱化为浅绿底+1px 内描边；`.content-view` 统一 1280px 居中容器；Hero 排版收敛（全站 56px、词书页 50px）。
- 学习页：学习工具从顶栏移入页内紧凑 `learning-context` 工具栏（顶栏只留长难句/查词/进度）；今日任务条改为文档流静默 Status Strip；Flashcard 620px/32px 圆角、单词 82px serif、提示文案精简为「点击 / Space 查看释义」。
- 词书/词本/测验精修：词书 Hero 缩小 10%、资源徽章降噪、全书乱序卡转「随机开始 →」行动入口；词本四项统计合并为整体 Stats Group、按钮分层（主要/次级/禁用分明）；测验卡垂直空白压缩约 10～15%。
- 轨迹页（唯一允许信息架构重构的页面）：21 个平铺模块重排为「今天 → 本周 → 未来 → 薄弱项 → 详细学习分析(默认折叠) → 历史」六段渐进披露；KPI 5→4（完成次数降为二级）、新增「本周最值得注意」薄弱高亮（0 值不占位）、查词行为与冲刺/cohort 系列全部收进折叠面板、薄弱集中区保持可见；数据零删除，仅移动/折叠/简化表现。
- 响应式：1920/1600/1440/1366/1180/1024/820/640/480 × 6 视图 DOM 扫描零溢出零冲突；e2e 新增幂等 `openTraceAnalysis` 辅助随渐进披露同步（13 个 signal-flow 用例前置展开，断言未降级）。
- 当前验证：lint 0 error / 1 个既有 warning、typecheck、production build、Node 单测全量 fail 0、完整 E2E **90/90**。
- 追加二（用户需求）：今日任务新词改为从**全书候选随机抽取**——候选过滤后先按日期种子洗牌再取前 N 个（此前只洗牌队列尾部、抽取源仍按书序取前 N），同日固定一批、次日自动换一批、已学排除、同词族错开保留；单测 64/64、完整 E2E 90/90。
- 追加修复（用户裁决）：最初 E2E 89/90 的唯一失败 `today-task-preview.spec.mjs:75` 经根因排查确认非业务 bug，而是「今日任务队列按日期种子整队乱序」与测试旧断言（首词=到期）冲突。用户裁决「只乱序新词、到期词置前」：`buildTodayQueueParts` 改为 `[dueIds, ...shuffle(priorityIds + newIds)]`，复习优先不被随机打乱，同种子确定性/跨批次一致/次日换新语义保留；单测更名并新增到期置前与尾部洗牌断言，e2e 改为逐词穷尽来源标签（dailyGoal 10 + 批次 15 使整队落入单批）。关键单测 196/196、完整 E2E 90/90。
- 追加三（用户报告）：学习页双层纵向滚动与词卡跳动——根因是 `.learn-view` 用 `min-height` 可被内容撑高（body 滚动）与 `.study-main-stack` 内部滚动叠加；`orbit-stage` 装饰圆环（620px/520px 绝对定位无 inset）溢出传播撑大 scrollHeight。修复：`.learn-view` 固定 `calc(100dvh - topbar)` 高度 + 兜底滚动、圆环 `overflow: clip` + `inset: 0; margin: auto` 居中裁剪、`rating-bar` 不可见时 `display: none` 不占位；切词后滚动位置由浏览器自然 clamp，无 JS 强制归零。
- 追加四（用户二次报告）：Study Workspace 自身滚动条彻底修复——stage `flex-shrink: 0` 防压缩、新增 `.card-viewport` 层（卡片高度由内容决定，clip 只裁圆环不裁卡片），修复 `align-items: stretch` 导致卡片被裁剪的次生问题（词根生成按钮被评分条拦截指针）。正常桌面 4 视口 stack 溢出 0px、切 20 词卡片中心 spread=0px；完整 E2E 90/90。
- 追加五（用户专项任务）：轨迹页「学习成长档案」视觉与信息架构优化——Hero 收敛 52px + 长期累计「N 次记忆记录」+ 副描述；今日四卡弱卡片化（已到期淡橙/牢固度淡绿）；学习趋势新增近 7 天每日学习量柱状图（纯展示层派生）；未来 30 天 flex 均分消除横向滚动（窄视口 min-width 640 内部滚动）；薄弱集中区 breakdown 默认收起、按钮改「复习 →」浅绿 pill；背诵日历热力图 75% + 右侧连续学习/最长连续/活跃日摘要；最近学习行 58px serif 20px + 评分状态点分层色；撤销按钮定位、绿色五档 token（--green-700/500/200/100/50）。1920 第一屏可见「本周」周报上半部分；完整 E2E 90/90。
- 服务：复用会话前 3000 端口本项目生产服务；验证期固定 3000 启动 dev（`.zcode/ui-round-dev.log`）并精确停止，最终重建 production build 恢复生产服务（`.zcode/ui-round-prod.log`，健康 200）；`lib/build-info.generated.ts` 已恢复为 HEAD 无漂移。
- 本轮改动未提交，等待用户确认提交方式；`next-round-prompt.md` 预写 P2-12 Prompt 已更正为「第 76 轮」，P2-12 仍需用户重新授权并执行新的 Round 0，不 push。

## 轨迹页「记忆档案」UI 精修与信息层级重构（第 78 轮）

本轮：2026-08-14。分支 `codex/follow-up-hardening`；起始 HEAD `75cd4d8`（工作区含用户既有的 `lib/build-info.generated.ts` 未提交改动，全程逐字节保留）。

- 纯展示层精修，未改学习算法 / 统计口径 / 持久化 / 交互回调：Hero 双指标同构数据组；本周报告改为「结论（36px 保持率/困难率 + 分子分母 + 较上周变化）→ 辅助指标 strip → 趋势 → 值得注意 → 建议」阅读顺序；当前状态 / 未来排程 / 近 7 日 / 划词集指标全部由独立卡改为间距 + 竖线分组；未来 30 天图只标注今天、每周节点、月份跨界与高峰（精确值保留在 aria-label/title）；薄弱集中区「复习 →」改为文字行动但保留 40×40 触控区，单元明细支持 `:focus-within` 展开。
- 新增确定性「本周诊断」（详细学习分析展开后首块）：只由现有 props 投影（最高计数薄弱信号、涉及词数、较上周变化、贡献最高分册建议、冲刺样本存在性），不新增 API/AI/缓存/统计函数，不输出因果结论；冲刺三区（观察 4 周 / 仍薄弱率 / 首次正常复习保持）仅在全为真正无样本时合并为一行说明，真实 0 与部分覆盖周照常展示。
- Level 1 Section 保留统一细边框，Level 2 Module 全部透明 + 顶部发丝线，Level 3 Metric 只用间距分组；全部复用既有 token，全局 `--content-max-width: 1280px` 未动（阅读宽度仅在 `.history-view` 局部收窄 1180px）；正文/说明 ≥11.5px，暖橙仅用于到期/困难/遗忘/薄弱。
- 背诵日历只压缩 padding/gap（`.activity-scroll` 24px 26px 20px → 14px 20px 10px），热力图格子、范围、图例→统计顺序、中心轴与移动端滚动契约全部保持；`tests/rendered-html.test.mjs` 仅同步该 padding 守卫值，断言强度未降。
- 同 seed 实测：页面总高折叠 -12.0% / 展开 -11.3%（未达 15–25% 目标，因结论块按要求放大与新增诊断内容，已如实记录）；背诵日历 -27%；卡片感元素 45→12（-73%）；<11px 文本 260→69（余下均为图表轴刻度）；9 视口无横向溢出。
- 验证：typecheck、eslint 0 error、rendered-html 12/12、review-trends 10/10、responsive 17/17、signal-flow 18/18、定向状态脚本 18/18（富数据/零数据诊断、暂无样本中性色、聚焦展开单元明细）；未运行 `npm test`（避免重建改写 build-info），`lib/build-info.generated.ts` 保持用户既有状态。
- 服务：复用固定端口 3000 既有本项目 `vinext dev`（PID 17660，健康 200），未重启、未换端口、未批量终止 Node；`1.txt`、`2.txt`、`.zcode/`、日志与截图零触碰。本轮完成后 STOP，不 push。
