...
## 第 75 轮追加五：轨迹页视觉与信息架构优化（用户专项任务）

- 目标：轨迹页从「数据库统计后台」气质转向「语言学习成长档案」——保留全部业务功能与数据语义，只改展示层。视觉关键词：克制、安静、长期主义、时间轴。
- Hero 重构：标题收敛至 52px（`clamp(40px, 3.8vw, 52px)`）+ 副描述「记录不是为了证明学过，而是为了知道什么真正留下来了。」；新增 Level 2 长期累计「N 次记忆记录」（serif 40px 品牌绿）；连续学习天数保持右侧同字号；`section-heading` margin 44→32px 消除第一屏无效空白。
- 今日四卡：从「单边框表格」改为四张弱卡片（`rgba(255,255,255,.6)` + 细边框 + radius 16，无阴影）；已到期卡极淡橙底（行动项）、牢固度卡极淡绿底（积极项）；数字 serif 34px、标题 12px secondary。
- 时间章节：`trace-section` 12px / letter-spacing .16em（今天/本周/未来/薄弱项/历史），保持「时间线」章节感。
- 周报：四周趋势卡当前周强调（绿色 32% 边框 + 2.5% 底，不过度变绿）；保持率绿 #55AD82、困难率 muted 橙 #D9895B（困难 ≠ 错误）；「本周值得注意」信号数字改 muted 橙，标题保持黑。
- 学习趋势：KPI 下方新增「学习量」近 7 天每日评分事件柱状图（纯展示层派生，不改变统计口径；无数据时 empty state）；「不同单词/完成次数」保留 insights-meta 条。
- 未来 30 天：柱状时间线改为 flex 均分（桌面 1152px 下 30 列全部 fit，**消除横向滚动**）；日期标签每 5 天显示 + hover title 保留；窄视口以 `min-width: 640px` 保护可读性（仅 forecast 内部横向滚动，不撑破页面）。
- 薄弱集中区：单元 breakdown 默认收起（opacity 0），悬停行时展开（180ms）；「冲刺」按钮改「复习 →」浅绿 pill（`rgba(47,127,96,.07)` 底 + 品牌绿字，不再深绿实底）；e2e 文案断言同步。
- 背诵日历：热力图与右侧新增「连续学习 / 最长连续 / 活跃日」摘要（75% + 25% 布局，消除右侧大片空白）；heatmap cell 11→12px；摘要数据仅由当前展示范围派生。
- 最近学习：行高 58px、单词 serif 20px、hover 极浅背景（`rgba(25,45,35,.015)`）；状态点按评分分层——忘记 orange / 模糊 amber / 认识 green / 熟练 mint（新增 rating-0 规则）。
- 其他：撤销按钮 right 28 / bottom 22；新增绿色五档 token（--green-700/500/200/100/50）并统一保持率/困难率/状态点/趋势柱；详细分析折叠条压缩至 58px 行高。
- 验证（Playwright 实测）：1366/1440/1920 下 forecast 无横向滚动、30 天全显、6 个日期标签；1920 第一屏依次可见 Hero → 今天 → 今日四卡 → 本周 → 周报上半部分；hover 集中区展开正常；320px 视口 forecast 内部滚动可访问（review-trends 用例保持）。typecheck、lint 0 error / 1 个既有 warning、完整 E2E **90/90**（含 review-trends 320px 可访问性用例）。
